...
const NewsletterForm = () => {
  ...
  return (
    <>
      {!submitStatus && (
        <form
          className="flex flex-col text-sm sm:flex-row sm:text-lg"
          method="POST"
          onSubmit={(e) => {
            e.preventDefault();
            const form = new FormData(e.target);
            const email = form.get("contact");
      ...
      <h3
        className={
          " w-full rounded-md bg-white p-3 text-center text-lg font-bold text-black " +
          (!submitStatus ? "hidden" : null)
        }
      >
        {submitStatus === "loading" && (
          <div role="status" className="flex items-center justify-center gap-3">
            ...
            <svg
              aria-hidden="true"
              className="h-7 w-7 animate-spin fill-black text-gray-200"
              viewBox="0 0 100 101"
              fill="none"
        ...
        {submitStatus === "success" && "Otrzymaliśmy twoje zgłoszenie!"}
        {submitStatus === "failure" && "Wystąpił nieoczekiwany błąd!"}
      </h3>
    </>
  );
};
